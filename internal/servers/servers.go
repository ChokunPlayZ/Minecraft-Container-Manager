...
type Server struct {
	...
	// SpinDownEnabled opts this server in to idle spin-down. When false (the
	// default), the scheduler never stops this server for being idle.
...
func (s *Store) List(ctx context.Context) ([]Server, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, name, server_type, version, COALESCE(build,''), ram_mb, cpu_limit, memory_limit_mb, host_port, COALESCE(extra_ports,'[]'), COALESCE(container_id,''), state, backup_enabled, backup_interval_minutes, spin_down_enabled, created_at, updated_at FROM servers ORDER BY created_at`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var srv Server
		var extra string
		if err := rows.Scan(&srv.ID, &srv.Name, &srv.ServerType, &srv.Version, &srv.Build, &srv.RAMMB, &srv.CPULimit, &srv.MemoryLimitMB, &srv.HostPort, &extra, &srv.ContainerID, &srv.State, &srv.BackupEnabled, &srv.BackupIntervalMinutes, &srv.SpinDownEnabled, &srv.CreatedAt, &srv.UpdatedAt); err != nil {
			return nil, err
		}
...
func (s *Store) Get(ctx context.Context, id string) (Server, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT id, name, server_type, version, COALESCE(build,''), ram_mb, cpu_limit, memory_limit_mb, host_port, COALESCE(extra_ports,'[]'), COALESCE(container_id,''), state, backup_enabled, backup_interval_minutes, spin_down_enabled, created_at, updated_at FROM servers WHERE id = ?`, id).
		Scan(&srv.ID, &srv.Name, &srv.ServerType, &srv.Version, &srv.Build, &srv.RAMMB, &srv.CPULimit, &srv.MemoryLimitMB, &srv.HostPort, &extra, &srv.ContainerID, &srv.State, &srv.BackupEnabled, &srv.BackupIntervalMinutes, &srv.SpinDownEnabled, &srv.CreatedAt, &srv.UpdatedAt)
...
func (s *Store) Update(ctx context.Context, id string, in UpdateInput) (Server, error) {
	...
	if in.SpinDownEnabled != nil {
		srv.SpinDownEnabled = *in.SpinDownEnabled
	}
	...
	now := time.Now().UTC().Format(time.RFC3339)
	_, err = s.db.ExecContext(ctx,
		`UPDATE servers SET name=?, server_type=?, version=?, build=?, ram_mb=?, host_port=?, cpu_limit=?, memory_limit_mb=?, backup_enabled=?, backup_interval_minutes=?, spin_down_enabled=?, extra_ports=?, container_id=?, state=?, updated_at=? WHERE id=?`,
		srv.Name, srv.ServerType, srv.Version, srv.Build, srv.RAMMB, srv.HostPort, srv.CPULimit, srv.MemoryLimitMB, srv.BackupEnabled, srv.BackupIntervalMinutes, srv.SpinDownEnabled, encodeExtraPorts(srv.ExtraPorts), srv.ContainerID, srv.State, now, id)
	if err != nil {
